...
class User:
    ...
    def store_payment_log(self,target,amount,note):
        # Bobby paid Carol $5.00 for Coffee
        self.stored_payment.append(f'{self.username} paid {target} ${float(amount)} for {note}')

    def add_friend_log(self,friend_username):


    def retrieve_feed(self):
        # retrieve also incoming payments for user TODO
        return self.stored_payment + self.friends

    def add_friend(self, new_friend):
        # check new_friend exists in the DB
        # check user is not himself
        self.friends.append(new_friend.username)
    ...
    def pay(self, target, amount, note):
        ...
        if self.balance > amount:
            # pay with balance
            pay_with_balance(target, amount, note)
        else:
            # pay with card
            pay_with_card(target, amount, note):
